...
class TgUsersNotifier:
    ...
    def notify(self, bot: Bot, filtered_data: list[FilteredResultDto]):
        users = self._tg_user_repository.get_all_active()

        for user in users:
            asyncio.run(bot.send_message(user.tg_id, text=self._format_message(filtered_data)))
    ...
    def _format_url(self, acc: list, item):
        ...
        return also(acc, lambda x: acc.append(next_val + '\n' + '=====' + '\n'))
